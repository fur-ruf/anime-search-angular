...
import { Anime, SearchFilters, SearchResult } from "../models";
import { createAnimeFuse } from "../config/fuse.config";
...
export class SearchService {
  ...
  constructor(jikanService: JikanService) {
    this.jikanService = jikanService;
    this.logger = console;
  }

  async smartSearch(filters: SearchFilters): Promise<SearchResult> {
    ...
    let searchQuery = q;
    if (q && /[а-яё]/i.test(q)) {
      searchQuery = transliterate(q);
      this.logger.info(`Транслитерация: "${q}" → "${searchQuery}"`);
    }

    this.logger.info(
      `Умный поиск: "${searchQuery || "все"}" с фильтрами`,
      filters,
    );

    const API_MAX_LIMIT = 25;

    let results: Anime[] = await this.jikanService.searchAnime({
  ...
  private applyFuzzySearch(data: Anime[], query: string): Anime[] {
    if (data.length === 0) return [];

    const fuse = createAnimeFuse(data);
  ...
  private applyFilters(
    ...
    if (filters.genres && filters.genres.length > 0) {
      ...
      this.logger.info(
        `После фильтра жанров: ${filtered.length} элементов (было ${before})`,
      );
    }
    ...
    if (filters.minScore !== undefined && filters.minScore !== null) {
      ...
      this.logger.info(
        `После фильтра по minScore (${filters.minScore}): ${filtered.length} элементов (было ${before})`,
      );
    }
    ...
    if (filters.maxScore !== undefined && filters.maxScore !== null) {
      ...
      this.logger.info(
        `После фильтра по maxScore (${filters.maxScore}): ${filtered.length} элементов (было ${before})`,
      );
    }
    ...
    if (filters.minYear) {
      ...
      this.logger.info(
        `После фильтра по minYear (${filters.minYear}): ${filtered.length} элементов (было ${before})`,
      );
    }
    ...
    if (filters.maxYear) {
      ...
      this.logger.info(
        `После фильтра по maxYear (${filters.maxYear}): ${filtered.length} элементов (было ${before})`,
      );
    }
    ...
    if (filters.status) {
      ...
      this.logger.info(
        `После фильтра по status (${filters.status}): ${filtered.length} элементов (было ${before})`,
      );
    }
    ...
    if (filters.type) {
      ...
      this.logger.info(
        `После фильтра по type (${filters.type}): ${filtered.length} элементов (было ${before})`,
      );
    }
    ...
    if (filters.minEpisodes) {
      ...
      this.logger.info(
        `После фильтра по minEpisodes (${filters.minEpisodes}): ${filtered.length} элементов (было ${before})`,
      );
    }
    ...
    if (filters.maxEpisodes) {
      ...
      this.logger.info(
        `После фильтра по maxEpisodes (${filters.maxEpisodes}): ${filtered.length} элементов (было ${before})`,
      );
    }

    this.logger.info(
      `Итог фильтрации: ${filtered.length} из ${data.length} элементов`,
    );
    return filtered;
  }
  ...
  private sortResults(
    ...
    switch (sortBy) {
      ...
      case "popularity":
        sorted.sort(
          (a, b) => (a.popularity || 999999) - (b.popularity || 999999),
        );
        break;
      case "rank":
  ...
  private generateSuggestions(results: Anime[], query?: string): string[] {
  ...
  async getRecommendations(id: number): Promise<Anime[]> {
    ...
    try {
      ...
      const executionTime = Date.now() - startTime;
      this.logger.info(
        `Найдено ${recommendations.length} рекомендаций за ${executionTime}мс`,
      );
